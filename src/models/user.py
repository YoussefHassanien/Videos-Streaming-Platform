...
class User(Base):
    ...
    # Reverse relationship - enables student.subscriptions access
    ...
    __table_args__ = (
        ...
        CheckConstraint("length(trim(email)) > 0",
                        name="check_email_not_empty"),
        CheckConstraint("length(password) >= 8",
                        name="check_password_min_length"),
        CheckConstraint("date_of_birth < CURRENT_TIMESTAMP",
                        name="check_date_of_birth_past"),
        CheckConstraint("length(mobile_number) >= 10",
